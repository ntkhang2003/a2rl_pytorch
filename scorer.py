...
class Scorer(object):
    def __init__(self, args):
        ...
        net_data = np.load(net_data_pth, encoding='latin1', allow_pickle=True).item()
        SPP = args.spp
        pooling = args.pooling
        global_dtype = tf.float32
        self.batch_size = 1
        self.image_placeholder = tf.compat.v1.placeholder(dtype=global_dtype, shape=[self.batch_size,self.img_h,self.img_w,3])
        var_dict = vfn_net.get_variable_dict(net_data)
        
        with tf.compat.v1.variable_scope("ranker") as scope:
