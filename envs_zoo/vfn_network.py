...
def score(feature_vec):
    W = tf.compat.v1.get_variable("W", shape=[feature_vec.get_shape()[1],1], initializer=tf.compat.v1.keras.initializers.VarianceScaling(distribution="uniform")) # init_weight([int(feature_vec.get_shape()[1]),1])
...
def get_variable_dict(net_data):
    variables_dict = {
        "c1w": tf.Variable(net_data["conv1"][0], trainable=False),
        "c1b": tf.Variable(net_data["conv1"][1], trainable=False),

        "c2w": tf.Variable(net_data["conv2"][0], trainable=False),
        "c2b": tf.Variable(net_data["conv2"][1], trainable=False),

        "c3w": tf.Variable(net_data["conv3"][0], trainable=False),
        "c3b": tf.Variable(net_data["conv3"][1], trainable=False),

        "c4w": tf.Variable(net_data["conv4"][0], trainable=False),
        "c4b": tf.Variable(net_data["conv4"][1], trainable=False),

        "c5w": tf.Variable(net_data["conv5"][0], trainable=False),
        "c5b": tf.Variable(net_data["conv5"][1], trainable=False)}
...
def build_alexconvnet(images, variable_dict, embedding_dim, SPP = False, pooling = 'max'):
    ...
    with tf.compat.v1.variable_scope("conv5"):
        k_h = 3; k_w = 3; s_h = 2; s_w = 2; padding = 'VALID'
        if pooling == 'max':
            pooling_func = tf.nn.max_pool2d
        else:
            pooling_func = tf.nn.avg_pool2d
        if SPP:
            ...
            concat5 = tf.concat([slim.flatten(maxpool1), slim.flatten(maxpool2), slim.flatten(maxpool3)], 1)
            bn5 = concat5
        else:
    ...
    fc6W =  tf.compat.v1.get_variable("fc6w", [flattened_dim, embedding_dim], initializer = tf.compat.v1.keras.initializers.VarianceScaling(distribution="uniform")) # init_weight((flattened_dim, embedding_dim))
    fc6b = tf.compat.v1.get_variable("fc6b", [embedding_dim], initializer = tf.compat.v1.constant_initializer())  #init_bias([embedding_dim])
